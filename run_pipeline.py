...
# ==========================================
# ⚙️ 1. 환경 설정 및 타이머 (일반 5.5시간 딥스크래핑 버전)
# ==========================================
START_TIME = time.time()
MAX_EXECUTION_TIME = 5.5 * 3600  # 5시간 30분 가동
...
SEASON_ID = 41
MATCHING_MODE = 3
MAX_LOOPS = 50000       # 대규모 수용을 위한 루프 횟수 대폭 상향
RECENT_GAME_LIMIT = 50  # 유저당 최근 50판 탐색
...
# ==========================================
# 🚀 2. 데이터 수집 (userId 기반 스노우볼)
# ==========================================
print("▶️ [1단계] 데이터 수집 시작 (Deep Scrape userId Mode)...")
...
if not queue:
    ...
    if res_rank.status_code == 200:
        ...
        print(f"🌐 [디버그] 가져온 상위 랭커 수: {len(top_ranks)}")
        
        for p in top_ranks[:200]: 
            nick = p.get("nickname")
...
while queue and loop_count < MAX_LOOPS:
    if time.time() - START_TIME > MAX_EXECUTION_TIME:
        print("⏱️ 5.5시간 제한 시간에 도달하여 수집 루프를 안전하게 종료합니다.")
        break
    ...
    try:
        # 1. 닉네임으로 유저 정보 조회하여 암호화된 'userId' 획득
        ...
        if res_user.status_code == 200:
            ...
            if "user" in user_json:
                ...
                user_id_str = user_obj.get("userId")
